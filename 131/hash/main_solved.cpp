...
public:
  ...
  void insert(K key, V value) {
    ...
    if (contains(key)) {
      throw std::logic_error("This key already exists, silly!");
    }
    destination.push_back(std::make_pair(key, value));
  }
  ...
  V at(K key) {
    ...
    // NOTE: This will never actually happen (as long as our logic is not
    // flawed)
    return destination[0].second;
  }
