...
Node *findMiddle(Node *head) {
  ...
  // Fast pointer must check itself and its next node to avoid segfaults
...
bool hasCycle(Node *head) {
  ...
  while (fast != nullptr && fast->next != nullptr) {
    ...
    // If they point to the same node in memory, there is a cycle
    if (slow == fast) {
      return true;
    }
  }

  // If the fast pointer reaches the end, it's a standard, non-cyclic list
  return false;
}
...
void runTests() {
  ...
  if (min1 && min1->data == 1) {
    cout << "[PASS] Test 1: Minimum of {10,2,8,1,5,9} is 1\n";
    passed++;
  } else {
    cout << "[FAIL] Test 1: Expected 1\n";
  }

  if (max1 && max1->data == 10) {
    cout << "[PASS] Test 2: Maximum of {10,2,8,1,5,9} is 10\n";
    passed++;
  } else {
    cout << "[FAIL] Test 2: Expected 10\n";
  }

  // Test 3 & 4: Min and Max with negative numbers
  Node *min2 = findMinimum(t4);
  Node *max2 = findMaximum(t4);
  if (min2 && min2->data == -10) {
    cout << "[PASS] Test 3: Minimum with negatives is -10\n";
    passed++;
  } else {
    cout << "[FAIL] Test 3: Expected -10\n";
  }

  if (max2 && max2->data == 3) {
    cout << "[PASS] Test 4: Maximum with negatives is 3\n";
    passed++;
  } else {
    cout << "[FAIL] Test 4: Expected 3\n";
  }
  ...
  if (findMinimum(emptyList) == nullptr && findMaximum(emptyList) == nullptr) {
    ...
    cout << "[FAIL] Test 5: Empty list should return nullptr\n";
  }

  // Test 6: Find middle of odd-length list
  Node *mid1 = findMiddle(t1);
  if (mid1 && mid1->data == 3) {
    cout << "[PASS] Test 6: Middle of {1,2,3,4,5} is 3\n";
    passed++;
  } else {
    cout << "[FAIL] Test 6: Expected 3\n";
  }

  // Test 7: Find middle of even-length list
  Node *mid2 = findMiddle(t2);
  if (mid2 && mid2->data == 4) {
    cout << "[PASS] Test 7: Middle of {1,2,3,4,5,6} is 4\n";
    passed++;
  } else {
    cout << "[FAIL] Test 7: Expected 4\n";
  }

  // Test 8: No cycle in a standard list
  if (!hasCycle(t1)) {
    cout << "[PASS] Test 8: No cycle detected in standard list\n";
    passed++;
  } else {
    cout << "[FAIL] Test 8: Falsely detected a cycle\n";
  }
  ...
  if (hasCycle(cycleList)) {
    cout << "[PASS] Test 9: Cycle successfully detected\n";
    passed++;
  } else {
    cout << "[FAIL] Test 9: Failed to detect cycle\n";
  }
