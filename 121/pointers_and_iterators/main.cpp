...
public:
  ...
  void add_item(int id, const std::string &name) {
    // --- YOUR CODE HERE ---
    // Hint: Use std::make_shared<Item>(...) and items.push_back(...)
  }
  ...
  void print_inventory() const {
    ...
    // --- YOUR CODE HERE ---
    // Hint: for (const auto& item_ptr : items) { ... }
    //       Access members using the -> operator (e.g., item_ptr->id)
  ...
  std::shared_ptr<Item> find_item_by_id(int id) {
    // --- YOUR CODE HERE ---
    // Hint: for (auto it = items.begin(); it != items.end(); ++it) { ... }
    //       Access the shared_ptr with *it
    //       Access the item's members with (*it)->id

    return nullptr; // Stub: remove this
  }
  ...
  bool remove_item_by_id(int id) {
    // --- YOUR CODE HERE ---
    // Hint: Same loop as TO-DO 3
    //       If found: items.erase(it); return true;

    return false; // Stub: remove this
  }
